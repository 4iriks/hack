"""
Pipeline v7: Customer profile features from pretrain (90M rows) + v6 features.

KEY INSIGHT: We have 90M pretrain rows but use NONE of them for customer profiling.
The model doesn't know each customer's "normal" behavior baseline.

New approach:
  1. Compute customer profiles from pretrain data (avg_amt, std_amt, typical patterns)
  2. For each train/test transaction, compute deviation from customer's profile
  3. Z-scores and outlier flags give the model explicit anomaly signals

New features (~17):
  - Customer profile: avg_amt, std_amt, med_amt, n_tx, n_unique_mcc,
    n_unique_channel, pct_night, pct_high_risk, avg_gap_sec, tenure_days
  - Deviation: amt_zscore, amt_vs_median, is_amt_outlier, dormancy_days, is_dormant

Strategy: labeled+green (proven on LB=0.094)
Base: v6 features (90) + customer profiles (~17) = ~107 features
"""
import polars as pl
import pandas as pd
import numpy as np
...
PRETRAIN    = ROOT / 'Pre-train_Train'
MODELS_OUT  = ROOT / 'models_v7'
...
# ═══════════════════════════════════════════
# STEP 0: Compute customer profiles from pretrain
# ═══════════════════════════════════════════

def compute_customer_profiles() -> pl.DataFrame:
    """
    Compute customer behavior profiles from pretrain data (90M rows).
    Uses Polars lazy scan — memory-efficient, no need to load all data.
    """
    cache_path = FEATURES_IN / 'customer_profiles.parquet'
    if cache_path.exists():
        log(f'Loading cached profiles from {cache_path}')
        return pl.read_parquet(cache_path)

    log('Computing customer profiles from pretrain (90M rows)...')

    pretrain_files = sorted(PRETRAIN.glob('pretrain_part_*.parquet'))
    log(f'Pretrain files: {[f.name for f in pretrain_files]}')

    # Lazy scan — reads only needed columns, processes in streaming mode
    lf = pl.concat([
        pl.scan_parquet(f).select([
            'customer_id', 'event_dttm', 'operaton_amt', 'mcc_code',
    ...
    lf = lf.with_columns(
        ...
        pl.col('operaton_amt').cast(pl.Float64, strict=False),
        pl.col('mcc_code').cast(pl.Int32, strict=False),
    )

    # Compute aggregates per customer
    log('Aggregating per customer...')
    profiles = lf.group_by('customer_id').agg([
        # Amount stats
        pl.col('operaton_amt').mean().alias('cust_avg_amt'),
        pl.col('operaton_amt').std().alias('cust_std_amt'),
        pl.col('operaton_amt').median().alias('cust_med_amt'),
        pl.col('operaton_amt').max().alias('cust_max_amt'),
        pl.col('operaton_amt').quantile(0.95).alias('cust_p95_amt'),

        # Activity stats
        pl.len().alias('cust_n_tx'),
        pl.col('mcc_code').n_unique().alias('cust_n_unique_mcc'),
        pl.col('channel_indicator_type').n_unique().alias('cust_n_unique_channel'),

        # Time patterns
        pl.col('event_dttm').dt.hour().mean().alias('cust_avg_hour'),
        pl.col('event_dttm').dt.hour().std().alias('cust_hour_std'),
        (pl.col('event_dttm').dt.hour() < 6).mean().alias('cust_pct_night'),

        # High-risk type fraction
        pl.col('event_type_nm').is_in(HIGH_RISK_TYPES).mean().alias('cust_pct_high_risk'),

        # Tenure
        pl.col('event_dttm').min().alias('cust_first_tx'),
        pl.col('event_dttm').max().alias('cust_last_tx'),
    ]).collect()

    log(f'Profiles computed: {len(profiles):,} customers')

    # Derived: tenure and frequency
    profiles = profiles.with_columns([
        ((pl.col('cust_last_tx') - pl.col('cust_first_tx')).dt.total_seconds() / 86400)
            .alias('cust_tenure_days'),
        # Average gap between transactions (seconds)
        ((pl.col('cust_last_tx') - pl.col('cust_first_tx')).dt.total_seconds() /
         (pl.col('cust_n_tx') - 1).clip(1, None))
            .alias('cust_avg_gap_sec'),
        # Last pretrain epoch for dormancy computation
        pl.col('cust_last_tx').dt.epoch('s').alias('cust_last_epoch'),
    ])

    # Drop datetime columns, keep numeric only
    profiles = profiles.drop(['cust_first_tx', 'cust_last_tx'])

    # Fill nulls for std (customers with 1 transaction)
    profiles = profiles.with_columns([
        pl.col('cust_std_amt').fill_null(0.0),
        pl.col('cust_hour_std').fill_null(0.0),
    ])

    profiles.write_parquet(cache_path)
    log(f'Saved profiles: {cache_path} ({profiles.estimated_size("mb"):.1f} MB)')
    return profiles


# ═══════════════════════════════════════════
# Feature Engineering (v6 base + customer deviations)
# ═══════════════════════════════════════════

def add_features(df: pl.DataFrame) -> pl.DataFrame:
    """Feature engineering v6 (90 features from v5+PDF)."""
    new_cols = []
...
def add_customer_profile_features(df: pl.DataFrame, profiles: pl.DataFrame) -> pl.DataFrame:
    """Join customer profiles and compute deviation features."""

    # Join profiles
    df = df.join(profiles, on='customer_id', how='left')

    # Deviation features: how unusual is this transaction for this customer?
    deviation_cols = []

    # Amount z-score: (current - mean) / std
    if 'cust_avg_amt' in df.columns and 'operaton_amt' in df.columns:
        deviation_cols.extend([
            ...
            (pl.col('operaton_amt') / (pl.col('cust_max_amt') + 1)).alias('amt_vs_max'),
            (pl.col('operaton_amt') / (pl.col('cust_p95_amt') + 1)).alias('amt_vs_p95'),
        ])
    ...
    if 'amt_zscore' in df.columns:
        df = df.with_columns([
            (pl.col('amt_zscore').abs() > 3.0).cast(pl.Int8).alias('is_amt_outlier'),
            (pl.col('amt_zscore').abs() > 2.0).cast(pl.Int8).alias('is_amt_unusual'),
        ])

    # Dormancy: days since last pretrain activity
    if 'cust_last_epoch' in df.columns and 'event_dttm' in df.columns:
        ...
        df = df.with_columns([
            (pl.col('dormancy_days') > 90).cast(pl.Int8).alias('is_dormant'),
        ])

    # Hour deviation from typical
    ...
    # Activity frequency ratio: current 30d frequency vs historical
    ...
    # Is new customer (not in pretrain)?
    if 'cust_n_tx' in df.columns:
        df = df.with_columns([
            pl.col('cust_n_tx').is_null().cast(pl.Int8).alias('is_new_customer'),
        ])

    # Fill nulls for customers not in pretrain
    profile_cols = [c for c in df.columns if c.startswith('cust_') or
                    c in ('amt_zscore','amt_vs_median','amt_vs_max','amt_vs_p95',
                           'is_amt_outlier','is_amt_unusual','dormancy_days','is_dormant',
...
FEATURE_COLS = [
    ...
    # v6 PDF features
    'is_high_risk_mcc', 'voip_and_rdp', 'n_security_flags',
    'mcc_novelty_x_amt', 'compromised_x_amt', 'manual_entry_x_amt',
    # v7 customer profile features
    ...
    # v7 deviation features
    'amt_zscore', 'amt_vs_median', 'amt_vs_max', 'amt_vs_p95',
    'is_amt_outlier', 'is_amt_unusual',
    'dormancy_days', 'is_dormant',
    'hour_deviation', 'freq_vs_historical', 'is_new_customer',
]
...
def main():
    t_start = time.time()
    print('\n' + '='*60)
    print('PIPELINE V7: Customer profiles + v6 features')
    print('='*60)

    # ─── Compute customer profiles ───
    profiles = compute_customer_profiles()
    log(f'Profiles: {len(profiles):,} customers, {len(profiles.columns)} cols')

    # ─── Load train features + enrich ───
    ...
    if 'is_new_customer' in available_feats:
        n_new_tr = df_tr['is_new_customer'].sum()
        n_new_val = df_val['is_new_customer'].sum()
        log(f'New customers (not in pretrain): train={n_new_tr}, val={n_new_val}')
    ...
    config = {
        'strategy': 'labeled_green_customer_profiles',
        'weights': weights,
    ...
    out_path = SUBMIT_OUT / f'submit_v7_{ts}.csv'
