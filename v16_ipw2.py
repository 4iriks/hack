"""
v16-IPW2: clip=50 and clip=100.
Trend: clip20 LB=0.1009 > clip10 LB=0.0981. Testing higher clips.
"""
import polars as pl
import pandas as pd
import numpy as np
...
def train_adversarial(X_train, X_test, feats):
    ...
    log('Training adversarial model (5-fold CV)...')
    ...
    log(f'  Adversarial OOF AUC: {roc_auc_score(y_adv, oof_preds):.4f}')
    ...
    log(f'  P(test|train) stats: mean={p_test.mean():.4f}, std={p_test.std():.4f}, '
...
def compute_ipw_weights(p_test, clip_max):
...
if __name__ == '__main__':
    ...
    # Test clip=50 and clip=100
    results = {}
    for clip in [50.0, 100.0]:
    ...
    log(f'\nBest: {best_name} (val={results[best_name]:.6f})')
    log(f'Reference: clip10=0.0981 LB, clip20=0.1009 LB')
    for n, p in sorted(results.items(), key=lambda x: -x[1]):
        log(f'  {n}: val={p:.6f}')

    # Generate submissions for both
    log('\n=== SUBMIT ===')
    ...
    for name in results:
        preds = np.mean([
            lgb.Booster(model_file=str(MODELS_OUT / f'{name}_s{s}.txt')).predict(X_te)
    ...
    with open(MODELS_OUT / 'results_v16_ipw2.json', 'w') as f:
